...
**CORS:** GAS Web Apps automatically handle CORS when deployed with "Anyone" access — responses include appropriate headers. The frontend uses `fetch()` with `mode: 'no-cors'` is NOT needed; GAS redirects require following redirects, so we use `fetch(url, {redirect: 'follow'})`.
...
- `?action=summary&months=6&user=all` — summary for last N months (totals by category per month)

**POST endpoints** (via JSON body `action` field):
- `{action: "addTransaction", id, date, amount, type, category, user, comment}` — add transaction
- `{action: "deleteTransaction", id}` — delete transaction by UUID
...
|--------|------|-------------|
| id | string (UUID) | Unique transaction ID (generated by frontend) |
| date | string (YYYY-MM-DD) | Transaction date |
| amount | number | Always positive. Type field determines income/expense. |
| type | string | "income" or "expense" (authoritative for sign) |
| category | string | Category name |
| user | string | User name |
| comment | string | Optional comment (max 100 chars) |
...
- **Tap to expand** transaction row → shows delete button (with confirmation dialog)
...
5. On success, frontend adds transaction to local state and re-renders. On error, shows toast — no optimistic update to avoid stale UI.
